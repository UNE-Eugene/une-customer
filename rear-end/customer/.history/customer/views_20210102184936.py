...
def city_date(form):
    with connection.cursor() as cursor:
        ...
        cursor.execute("SELECT suggestion from T_Hotel_Suggestion inner join T_Hotel_Info where T_Hotel_Info.city=%s and T_Hotel_Suggestion.hotel_id = T_Hotel_Info.hotel_id", [form['city']]) #酒店备注查找
        cursor.execute("SELECT tags from T_Hotel_Tags inner join T_Hotel_Info where T_Hotel_Info.city=%s and T_Hotel_Tags.hotel_id = T_Hotel_Info.hotel_id", [form['city']]) 
        row = cursor.fetchone()
        for item in row:
            print(item)
