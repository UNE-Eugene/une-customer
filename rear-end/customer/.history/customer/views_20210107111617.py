...
def ask(request):
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor:
            ...
            if result is None:
                cursor.execute("INSERT INTO T_Hotel_Asked(message, remarks, promoter, accepter, askid, update_time) Values(%s,%s,%s,%s,%s,%s)", [form['message'], form['remark'], username.username, relation, form['askid'], datetime.now()])
            else:
...
def get_ask_log(request):
    if request.method == 'POST':
        ...
        askid = form['askid']
        with connection.cursor() as cursor:
            cursor.execute('select * from T_Hotel_Asked where     ')