...
def city_trade(form):
    result = []
    with connection.cursor() as cursor:
        cursor.execute("SELECT hotel_id, hotel_name, data_url FROM T_Hotel_Info inner join T_Hotel_Trade_Distance where T_Hotel_Info.city=%s and T_Hotel_Trade_Distance.hotel_id=T_Hotel_Info.hotel_id and T_Hotel_Trade_Distance.name=%s and T_Hotel_Trade_Distance.distance<5", [form['city'], form['trade']]) #酒店查找
        message = cursor.fetchall()
        cursor.execute("SELECT suggestion from (T_Hotel_Suggestions inner join T_Hotel_Info on T_Hotel_Info.city=%s and T_Hotel_Suggestions.hotel_id = T_Hotel_Info.hotel_id) inner join T_Hotel_Trade_Distance on T_Hotel_Trade_Distance.hotel_id=T_Hotel_Suggestion.hotel_id and T_Hotel_Trade_Distance.name=%s and T_Hotel_Trade_Distance.distance<5", [form['city'], form['trade']]) #酒店备注查找
        suggestion = cursor.fetchall()
        cursor.execute("SELECT tags from T_Hotel_Tag inner join T_Hotel_Info where T_Hotel_Info.city=%s and T_Hotel_Tag.hotel_id = T_Hotel_Info.hotel_id", [form['city']]) 
        tags = cursor.fetchall()
        cursor.execute("SELECT address from T_Hotel_Address inner join T_Hotel_Info where T_Hotel_Info.city=%s and T_Hotel_Address.hotel_id = T_Hotel_Info.hotel_id", [form['city']]) 
