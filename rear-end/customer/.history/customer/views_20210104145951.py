...
def city_trade(form):
    ...
    with connection.cursor() as cursor:
        ...
        message = cursor.fetchall()
        cursor.execute("SELECT suggestion from T_Hotel_Suggestions inner join T_Hotel_Trade_Distance on T_Hotel_Trade_Distance.city=%s and T_Hotel_Trade_Distance.hotel_id=T_Hotel_Suggestion.hotel_id and T_Hotel_Trade_Distance.name=%s and T_Hotel_Trade_Distance.distance<5", [form['city'], form['trade']]) #酒店备注查找
        suggestion = cursor.fetchall()
