...
def hotel(form):
    ...
    with connection.cursor() as cursor:
        ...
        suggestion = cursor.fetchall()
        cursor.execute("SELECT tags from T_Hotel_Tag inner join T_Hotel_Info where T_Hotel_Info.hotel_name=%s and T_Hotel_Info.hotel_id=T_Hotel_Tags.hotel_id", [form['hotel']])
        tags = cursor.fetchall()
...
def search(request):
    if request.method == 'POST':
        ...
        if form['city'] != '' and form['hotel'] == '':
            ...
            print(form['hotel'].endswith('酒店'))
            if form['hotel'].endswith('酒店'):
                pass
            else:
                form['hotel'] = form['hotel'] + '酒店'
            print(form)
            hotel(form)
    return HttpResponse() 