...
def ask(request):
    if request.method == 'POST':
        ...
        print(type(username))
        with connection.cursor() as cursor:
            cursor.execute('SELECT relation from T_User_Message where user=%s', [username])
            relation = cursor.fetchone()
            print(relation)
            cursor.execute("INSERT INTO T_Hotel_Ask(message, remarks, promoter, accepter) Values(%s,%s,%s,%s)", [form['ask'], form['remark'], username, relation])
