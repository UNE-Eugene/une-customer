...
function tagRender(props) {
  ...
  return (
    label===null?null:
    <Tag color={value} closable={closable} onClose={onClose} style={{ color: 'black', marginRight: 3 }}>
      {label}
    </Tag>
  );
}
                ...
                return (
                  ...
                  >
                    <Button key={index} type="text" onClick={()=>{
                      formData.city = item;
                      setChosen([formData.city, formData.hotel,formData.date[0],formData.date[1], formData.trade, formData.group, formData.budget])
                    }}>
                      {item}
                    </Button>
                  </Col>
