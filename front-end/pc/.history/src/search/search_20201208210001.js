...
const Search = (props) => {
  ...
  const [chosen, setChosen] = useState(["杭州", "杭州洲际酒店", '2020/12/11', '来福士']);
    ...
    >
      ...
      <Card className="card">
        ...
        >
          <Form.Item label="城市" name='city'>
              <Row gutter={[16, 20]}>
                  {chosen.map((item, index)=>{
                      return(
                          <Col>
                            <Button key={index} onClick={()=>{chosen.push(item)}}>{item}</Button>
                          </Col>
                      )
                  })}
                  <Col><Input /></Col>
          ...
          <Form.Item label="酒店" name='hotel'></Form.Item>
          <Form.Item label="日期" name='date'></Form.Item>
          <Form.Item label="商圈" name='trade'></Form.Item>
          <Form.Item label="集团" name='group'></Form.Item>
        </Form>
      </Card>
    </div>
  );
